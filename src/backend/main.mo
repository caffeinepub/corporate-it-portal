...
actor {
  ...
  type OldUserProfile = {
    principal : Principal; name : Text; dateOfBirth : Text; email : Text;
    mobile : Text; country : Text; roleTitle : Text; registrationPurpose : Text;
    rejectionReason : ?Text; status : OldUserStatus;
  };
  ...
  public type RegistrationRecord = {
    id : Text; name : Text; dateOfBirth : Text; email : Text; mobile : Text;
    country : Text; roleTitle : Text; registrationPurpose : Text;
    deviceInfo : Text; timestamp : Int; status : RegistrationStatus;
    rejectionReason : ?Text; loginUsername : ?Text; loginPassword : ?Text;
  };
  public type FailedLog = {
    email : Text; roleTitle : Text; errorMsg : Text; deviceInfo : Text; timestamp : Int;
  };
  public type RegistrationStats = {
    total : Nat; pending : Nat; approved : Nat; rejected : Nat; failed : Nat;
  };
  public type BookingStatus = { #pending; #approved; #rejected };
  public type BookingRecord = {
    id : Text; roomName : Text; roomType : Text; bookerName : Text; panName : Text;
    dob : Text; mobile : Text; email : Text; purpose : Text; bookingDate : Text;
    bookingHour : Text; bookingMinute : Text; bookingSecond : Text; timestamp : Int;
    status : BookingStatus; approvedBookingId : ?Text; approvedAt : ?Int;
    rejectedAt : ?Int; rejectionReason : ?Text;
  };
  public type BookingStats = { total : Nat; pending : Nat; approved : Nat; rejected : Nat; };
  public type ServiceRating = {
    id : Text; bookingId : Text; roomName : Text; bookerName : Text; email : Text;
    selectedOptions : [Text]; overallComment : Text; timestamp : Int;
  };
  ...
  func simpleHash(s : Text) : Nat {
    var h : Nat = 5381;
    for (c in s.chars()) { h := (h * 33 + c.toNat32().toNat()) % 1_000_000_007; };
    h;
  };
  ...
  func isValidToken(token : Text) : Bool {
    switch (adminSessions.get(token)) { case (?_) { true }; case null { false }; };
  };

  // UPDATE ping -- properly wakes the canister for update calls
  public func ping() : async Bool { true };
  ...
  public query func verifyAdminToken(token : Text) : async Bool { isValidToken(token) };
  public func logoutAdmin(token : Text) : async () { adminSessions.remove(token) };

  public func submitRegistration(
    name : Text, dateOfBirth : Text, email : Text, mobile : Text,
    country : Text, roleTitle : Text, registrationPurpose : Text, deviceInfo : Text,
  ) : async Text {
    let ts = Time.now();
    regCounter += 1;
    let id = generateId("REG", regCounter, ts);
    let record : RegistrationRecord = {
      id = id; name = name; dateOfBirth = dateOfBirth; email = email;
      mobile = mobile; country = country; roleTitle = roleTitle;
      registrationPurpose = registrationPurpose; deviceInfo = deviceInfo;
      timestamp = ts; status = #pending; rejectionReason = null;
  ...
  public query func getRegistrationStatusByEmail(email : Text) : async ?RegistrationRecord {
    ...
    for (rec in registrations.values()) {
      if (rec.email == email and rec.timestamp > latestTs) { latestTs := rec.timestamp; found := ?rec; };
    };
    found;
  };
  public query func getRegistrationStatusByMobile(mobile : Text) : async ?RegistrationRecord {
    ...
    var latestTs : Int = 0;
    for (rec in registrations.values()) {
      if (rec.mobile == mobile and rec.timestamp > latestTs) { latestTs := rec.timestamp; found := ?rec; };
    };
    found;
  };
  ...
  public func approveRegistration(token : Text, regId : Text) : async Bool {
    ...
    switch (registrations.get(regId)) {
      ...
      case (?rec) {
        ...
        let updated : RegistrationRecord = { rec with status = #approved; rejectionReason = null; loginUsername = ?userId; loginPassword = ?pwd };
  ...
  public func rejectRegistration(token : Text, regId : Text, reason : Text) : async Bool {
    ...
    switch (registrations.get(regId)) {
      case null { false };
      case (?rec) {
        registrations.add(regId, { rec with status = #rejected; rejectionReason = ?reason });
        true;
      };
    };
  };
  public func logFailedRegistration(email : Text, roleTitle : Text, errorMsg : Text, deviceInfo : Text) : async () {
    let ts = Time.now();
    logCounter += 1;
    let id = generateId("LOG", logCounter, ts);
    failedLogs.add(id, { email = email; roleTitle = roleTitle; errorMsg = errorMsg; deviceInfo = deviceInfo; timestamp = ts });
  };
  ...
  public query func getRegistrationStats(token : Text) : async RegistrationStats {
    if (not isValidToken(token)) { return { total = 0; pending = 0; approved = 0; rejected = 0; failed = 0 } };
    var total = 0; var pending = 0; var approved = 0; var rejected = 0;
    for (rec in registrations.values()) {
      total += 1;
      switch (rec.status) { case (#pending) { pending += 1 }; case (#approved) { approved += 1 }; case (#rejected) { rejected += 1 }; };
    };
    { total; pending; approved; rejected; failed = failedLogs.size() };
  };

  public func submitBooking(
    roomName : Text, roomType : Text, bookerName : Text, panName : Text,
    dob : Text, mobile : Text, email : Text, purpose : Text,
    bookingDate : Text, bookingHour : Text, bookingMinute : Text, bookingSecond : Text,
  ) : async Text {
    ...
    let record : BookingRecord = {
      id = id; roomName = roomName; roomType = roomType; bookerName = bookerName;
      panName = panName; dob = dob; mobile = mobile; email = email; purpose = purpose;
      bookingDate = bookingDate; bookingHour = bookingHour; bookingMinute = bookingMinute;
      bookingSecond = bookingSecond; timestamp = ts; status = #pending;
      approvedBookingId = null; approvedAt = null; rejectedAt = null; rejectionReason = null;
    };
  ...
  public query func getBookingById(id : Text) : async ?BookingRecord { bookings.get(id) };
  ...
  public func approveBooking(token : Text, bookingId : Text) : async Bool {
    ...
    switch (bookings.get(bookingId)) {
      case null { false };
      case (?rec) {
        let approvedId = "EBC" # (bookingCounter + 1000).toText();
        let ts = Time.now();
        let updated : BookingRecord = { rec with status = #approved; approvedBookingId = ?approvedId; approvedAt = ?ts; rejectionReason = null };
  ...
  public func rejectBooking(token : Text, bookingId : Text, reason : Text) : async Bool {
    ...
    switch (bookings.get(bookingId)) {
      ...
      case (?rec) {
        let ts = Time.now();
        bookings.add(bookingId, { rec with status = #rejected; rejectedAt = ?ts; rejectionReason = ?reason });
        true;
      };
    };
  };
  public query func getBookingStats(token : Text) : async BookingStats {
    if (not isValidToken(token)) { return { total = 0; pending = 0; approved = 0; rejected = 0 } };
    var total = 0; var pending = 0; var approved = 0; var rejected = 0;
    for (b in bookings.values()) {
      total += 1;
      switch (b.status) { case (#pending) { pending += 1 }; case (#approved) { approved += 1 }; case (#rejected) { rejected += 1 }; };
    };
    { total; pending; approved; rejected };
  };

  public func submitRating(
    bookingId : Text, roomName : Text, bookerName : Text, email : Text,
    selectedOptions : [Text], overallComment : Text,
  ) : async Text {
    let ts = Time.now();
    ratingCounter += 1;
    let id = generateId("RTG", ratingCounter, ts);
    ratings.add(id, { id = id; bookingId = bookingId; roomName = roomName; bookerName = bookerName; email = email; selectedOptions = selectedOptions; overallComment = overallComment; timestamp = ts });
    id;
  };
